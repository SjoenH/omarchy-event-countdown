...
Panel {
  ...
  // Edit-form fields (bound to the selected event).
  ...
  function deepCopyEvents(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
      var e = arr[i]
      out.push({
        id: e.id, name: e.name, month: e.month, day: e.day,
        year: e.year, repeats: e.repeats
      })
    }
    return out
  }
  ...
  function current() {
    if (root.selIndex < 0 || root.selIndex >= root.workEvents.length) return null
  ...
  function applySelection() {
    var e = root.current()
    if (!e) return
    if (root.edName !== e.name) { e.name = root.edName; root.dirty() }
    if (root.edMonth !== e.month) { e.month = root.edMonth; root.dirty() }
    if (root.edDay !== e.day) { e.day = root.edDay; root.dirty() }
    if (root.edYear !== e.year) { e.year = root.edYear; root.dirty() }
    if (root.edRepeats !== e.repeats) { e.repeats = root.edRepeats; root.dirty() }
  }

  // Live preview (in-memory bar update, no disk write).
  function dirty() {
    if (root.hostWidget) root.hostWidget.previewEvents(root.workEvents)
  }
  ...
  function canCommit() {
    ...
    if (e.day < 1 || e.day > 31) return false
    if (!e.repeats && (e.year < 1 || e.year > 9999)) return false
    return true
  }
  ...
  function commit() {
    ...
    root.hostWidget.persist({ events: root.workEvents })
  }

  function switchMode(mode) {
    if (mode === root.edMode) return
    root.edMode = mode
    if (root.hostWidget) root.hostWidget.persist({ mode: mode })
  }
  ...
  function open() { root.syncFromWidget(); root.controller.show() }
  function close() { root.applySelection(); if (root.canCommit()) root.commit(); root.controller.hide() }
  ...
  function fracOf(e) { return root.hostWidget ? root.hostWidget.fractionOf(e) : 0 }
  function countOf(e) { return root.hostWidget ? root.hostWidget.countOf(e) : { text: "", upcoming: true } }
  function nameOf(e) { return root.hostWidget ? root.hostWidget.monthDayName(e) : "" }
  function barBg() { return root.bar ? root.bar.background : Color.background }
      ...
      Column {
        ...
        Column {
          ...
          ButtonGroup {
            ...
            onChanged: function(mode) { root.switchMode(mode) }
          }
        }

        // --- Events list (minimalistic filled progress rows) ---
        Column {
          width: parent.width
                    ...
                    Row {
                      ...
                      Row {
                        ...
                        Rectangle {
                          ...
                          height: 7
                          radius: 4
                          color: root.countOf(modelData).upcoming ? "#00cc66" : "#888888"
                        }
                      }
                    }
                  }
        ...
        Column {
          ...
          Column {
            ...
            TextField {
              ...
              font.family: root.bar ? root.bar.fontFamily : Style.font.family
              font.pixelSize: Style.font.body
              onTextChanged: { root.edName = text; root.applySelection() }
          ...
          Row {
            ...
            Column {
              ...
              NumberField {
                ...
                accent: Color.accent
                fontFamily: root.bar ? root.bar.fontFamily : Style.font.family
                onModified: function(v) { root.edMonth = v; root.applySelection() }
              }
            }
            ...
            Column {
              ...
              NumberField {
                ...
                accent: Color.accent
                fontFamily: root.bar ? root.bar.fontFamily : Style.font.family
                onModified: function(v) { root.edDay = v; root.applySelection() }
              }
            }
            ...
            Column {
              ...
              NumberField {
                ...
                accent: Color.accent
                fontFamily: root.bar ? root.bar.fontFamily : Style.font.family
                onModified: function(v) { root.edYear = v; root.applySelection() }
              }
            }
          }
        ...
        // --- Add-only row when no event selected ---
        Button {
          width: parent.width
          visible: root.current() === null
          text: "Add event"
